...
int G(int i, int j) {
	return stride+j*g_width+i;
}
int B(int i, int j) {
	return stride+stride+j*g_width+i;
}
...
void darken (unsigned char in[], unsigned char out[], int width, int height) {
	...
	for (int i = 0; i < width; i++) { 
		for (int j = 0; j < height; j++) { //At each pixel...
			out[R (i, j)] = in[R (i, j)] / 2; //Divide the value of red by half...
			out[G (i, j)] = in[G (i, j)] / 2; //...and green...
			out[B (i, j)] = in[B (i, j)] / 2; //...and blue.
		}
	}
...
int main (int argc, char **argv) {
	...
	*/
	...
	//Re-enable this sepia_image.save_jpeg ("output.jpg", 50); //50% quality 
	end_time = clock();
